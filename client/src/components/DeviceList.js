"use strict";
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator["throw"](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g = Object.create(
        (typeof Iterator === "function" ? Iterator : Object).prototype,
      );
    return (
      (g.next = verb(0)),
      (g["throw"] = verb(1)),
      (g["return"] = verb(2)),
      typeof Symbol === "function" &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError("Generator is already executing.");
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y["return"]
                  : op[0]
                    ? y["throw"] || ((t = y["return"]) && t.call(y), 0)
                    : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
Object.defineProperty(exports, "__esModule", { value: true });
exports.DeviceList = void 0;
var react_1 = require("react");
var deviceService_1 = require("../services/deviceService");
var useWebSocket_1 = require("../hooks/useWebSocket");
var DeviceCard_1 = require("./DeviceCard");
var DeviceList = function (_a) {
  var token = _a.token;
  var _b = (0, react_1.useState)([]),
    devices = _b[0],
    setDevices = _b[1];
  var _c = (0, react_1.useState)(true),
    loading = _c[0],
    setLoading = _c[1];
  var _d = (0, react_1.useState)(null),
    error = _d[0],
    setError = _d[1];
  var deviceService = new deviceService_1.DeviceService(token);
  var _e = (0, useWebSocket_1.useWebSocket)(token),
    connected = _e.connected,
    subscribe = _e.subscribe;
  (0, react_1.useEffect)(
    function () {
      var loadDevices = function () {
        return __awaiter(void 0, void 0, void 0, function () {
          var fetchedDevices, err_1;
          return __generator(this, function (_a) {
            switch (_a.label) {
              case 0:
                _a.trys.push([0, 2, 3, 4]);
                return [4 /*yield*/, deviceService.getAllDevices()];
              case 1:
                fetchedDevices = _a.sent();
                setDevices(fetchedDevices);
                setError(null);
                return [3 /*break*/, 4];
              case 2:
                err_1 = _a.sent();
                setError("Failed to load devices");
                console.error("Error loading devices:", err_1);
                return [3 /*break*/, 4];
              case 3:
                setLoading(false);
                return [7 /*endfinally*/];
              case 4:
                return [2 /*return*/];
            }
          });
        });
      };
      loadDevices();
    },
    [deviceService],
  );
  (0, react_1.useEffect)(
    function () {
      if (connected) {
        subscribe(
          // Device update handler
          function (updatedDevice) {
            setDevices(function (prevDevices) {
              return prevDevices.map(function (device) {
                return device.id === updatedDevice.id ? updatedDevice : device;
              });
            });
          },
          // Automation event handler
          function (event) {
            console.log("Automation event:", event);
          },
          // Error handler
          function (error) {
            console.error("WebSocket error:", error);
            setError("Connection error");
          },
        );
      }
    },
    [connected, subscribe],
  );
  var handleDeviceCommand = function (deviceId, command) {
    return __awaiter(void 0, void 0, void 0, function () {
      var err_2;
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            _a.trys.push([0, 2, , 3]);
            return [
              4 /*yield*/,
              deviceService.executeCommand(deviceId, command),
            ];
          case 1:
            _a.sent();
            setError(null);
            return [3 /*break*/, 3];
          case 2:
            err_2 = _a.sent();
            setError("Failed to execute command");
            console.error("Error executing command:", err_2);
            return [3 /*break*/, 3];
          case 3:
            return [2 /*return*/];
        }
      });
    });
  };
  if (loading) {
    return <div>Loading devices...</div>;
  }
  if (error) {
    return (
      <div className="error-message">
        {error}
        <button
          onClick={function () {
            return setError(null);
          }}
        >
          Dismiss
        </button>
      </div>
    );
  }
  return (
    <div className="device-list">
      <div className="device-list-header">
        <h2>Your Devices</h2>
        <div className="connection-status">
          {connected ? (
            <span className="status-connected">Connected</span>
          ) : (
            <span className="status-disconnected">Disconnected</span>
          )}
        </div>
      </div>

      <div className="device-grid">
        {devices.length === 0 ? (
          <div className="no-devices">
            No devices found. Click "Add Device" to get started.
          </div>
        ) : (
          devices.map(function (device) {
            return (
              <DeviceCard_1.default
                key={device.id}
                device={device}
                onCommand={handleDeviceCommand}
              />
            );
          })
        )}
      </div>
    </div>
  );
};
exports.DeviceList = DeviceList;

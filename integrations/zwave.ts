...
class ZWaveController extends EventEmitter {
   ...
   * Parse Z-Wave protocol data
   */
  private parseZWaveData(data: Buffer): void {
  ...
  private prepareLightCommand(device: any, command: any): number[] {
    ...
    if (command.on !== undefined) {
      ...
      const level = Math.max(
        0,
        Math.min(99, Math.round(command.brightness * 0.99)),
      );
   ...
   */
  private prepareLockCommand(device: any, command: any): number[] {
    const nodeId = device.nodeId;
...
// Singleton instance
let controller: ZWaveController | null = null;
